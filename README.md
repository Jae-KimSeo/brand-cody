...
### 4. 브랜드 및 상품 관리
- 새로운 브랜드를 등록할 수 있습니다.
- 브랜드별 카테고리에 여러 상품을 추가하여 다양한 가격대의 제품을 제공할 수 있습니다.
- 각 브랜드의 카테고리별 상품을 추가, 변경, 삭제할 수 있습니다.
...
- **상품 다양화**: 브랜드당 각 카테고리에 여러 상품 등록이 가능하여 다양한 가격대와 옵션을 제공합니다.
- **데이터 제약조건**: 브랜드 이름에 유니크 제약조건을 적용하여 브랜드 데이터 무결성을 보장합니다.
- **인덱스 최적화**: 
...
- **다중 상품 동시 생성**: 동일 브랜드-카테고리 조합으로 다수의 상품을 동시에 생성할 수 있어 시스템 확장성이 향상되었습니다.
- **자동 재시도 메커니즘**: Spring Retry를 사용하여 낙관적 락 충돌 시 자동으로 재시도하는 로직을 구현했습니다.
- **동시성 테스트**: 여러 스레드가 동시에 다양한 상품 조작을 수행하는 테스트를 통해 동시성 처리를 검증했습니다.
...
2. **단일 브랜드로 모든 카테고리 구매 시 최저가 브랜드 조회**:
   - 브랜드별 카테고리당 최저가 상품만을 선택하여 총액 계산
...
4. **브랜드 및 상품 CRUD 기능**:
   ...
   - 브랜드당 카테고리별 다중 상품 관리 지원으로 상품 라인업 확장 가능
   - 다중 상품 중 최저가 상품을 선별하는 지능형 업데이트 로직 구현
...
- **다중 상품 테스트**: 동일 브랜드-카테고리 조합에 여러 상품 생성 및 관리 검증
- **동시성 테스트**: 여러 스레드를 사용한 동시 상품 생성 및 업데이트 시나리오 검증
- **지능형 가격 선택 검증**: 다중 상품 상황에서 최저가 상품 선택 로직 테스트
